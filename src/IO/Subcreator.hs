...
begin u = do    let r = uResolution u
                r' <- IO.Subcreator.initWindow (uName u) r supportedResolutions
                initAudioDevice
                ...
                toggleFullScreen
                setTargetFPS (uFps u)
                let u' = u { uScaleFactor = Core.Visual.scaleFactor r r' }
...
end (tm, fm, sm, mm) = do mapM_ (unloadTexture . snd) (toList tm)
...
keysTouched :: [(IO.Raylib.KeyboardKey, Core.Tactile.KeyboardKey)] -> (IO.Raylib.KeyboardKey -> IO Bool) -> IO (Set Core.Tactile.KeyboardKey)
